...
export default class extends Controller<'year' | 'month' | 'day'> {
  ...
  public async main(
    ...
    @Controller.Param('month', Number) month: number,
    @Controller.Param('day', Number) day: number,
  ) {
    ...
    const state = await Promise.resolve(theme.state({ page, url, year, month, day, type, category }, context));
    return new Response()
